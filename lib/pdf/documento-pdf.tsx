// Componente de render del PDF de gastos — diseño "Marfil" APROBADO por LM Aduanas.
// Portado desde docs/DocumentoPDF.jsx (sin cambios de diseño: layout v3 aprobado).
// Ramificado por `data.tipo`. Recibe el snapshot de `document_versions.data`.
//
// Uso (en la API route):
...
//   )
//
// No se almacena el PDF (ver arquitectura §6): se genera al vuelo en cada exportación.
...
import type { DocumentoData, DatosVehiculo, DatosContenedor } from '@/lib/documentos/types'
...
const styles = StyleSheet.create({
  ...
  brandTextWrap: { borderLeftWidth: 1, borderLeftColor: COLORES.line, paddingLeft: 10, maxWidth: 150 },
  ...
  dataLabel: { fontSize: TIPOGRAFIA.tamanos.label - 1.5, fontWeight: 700, color: COLORES.muted, textTransform: 'uppercase' },
  ...
  dueValor: { fontSize: TIPOGRAFIA.tamanos.dueValor, fontWeight: 800, color: COLORES.dueText },

  // ---- Tabla de gastos ----
  tbl: { marginTop: 14, borderWidth: 1, borderColor: COLORES.line, borderRadius: MEDIDAS.radioBorde, overflow: 'hidden' },
  tblHeader: { flexDirection: 'row', backgroundColor: COLORES.tableHeaderBg, borderBottomWidth: 2, borderBottomColor: COLORES.brandInk },
  ...
  filaTotal: { flexDirection: 'row', backgroundColor: COLORES.totalBg, borderTopWidth: 2, borderTopColor: COLORES.brandInk },
  totalLabel: {
    flex: 1,
    paddingVertical: 9,
  ...
  noteTexto: { flex: 1, fontSize: TIPOGRAFIA.tamanos.nota, fontWeight: 700, color: COLORES.noteInk, lineHeight: 1.4 },
...
function IconoAdvertencia({ size = MEDIDAS.iconoAdvertencia }: { size?: number }) {
...
function BloqueVehiculo({ vehiculo }: { vehiculo?: DatosVehiculo }) {
  return (
...
function BloqueContenedor({ contenedor }: { contenedor?: DatosContenedor }) {
  return (
    <View style={styles.dataBlock}>
      <View style={styles.dataCell}>
...
export type DocumentoPDFProps = {
  ...
  logoBase64?: string // base64 PNG (sin prefijo data:) del logo de la org
...
export default function DocumentoPDF({
  ...
  const fechaISO =
    typeof fechaEmision === 'string' ? fechaEmision : fechaEmision.toISOString().slice(0, 10)
      ...
      <Page size="LETTER" style={styles.page}>
        ...
        <View style={styles.hd}>
          ...
          <View style={styles.metaWrap}>
            <Text style={styles.metaPag}>
              Pág. {paginaActual} of {totalPaginas}
            </Text>
            <Text style={styles.metaFecha}>Date: {formatoFecha(fechaISO)}</Text>
          </View>
        </View>
        ...
        <View style={styles.tbl}>
          <View style={styles.tblHeader}>
            <View style={styles.thConcepto}>
              <Text style={styles.thTexto}>Concepto</Text>
            </View>
            <View style={styles.thMonto}>
              <Text style={[styles.thTexto, styles.thTextoRight]}>Monto</Text>
            </View>
          </View>
          {conceptos.map((c, i) => (
            <View key={i} style={[styles.fila, i === 0 ? styles.filaPrimera : {}]}>
